...
const TARGET_CODES = ['11427', '28750', '225', '191', '192']
...
async function main() {
  const series = await db
    ...
    .where(inArray(macroSeriesMeta.sourceCode, TARGET_CODES))

  console.log(`Backfill de ${series.length} series (${toDateStr(START)} — ${toDateStr(END)})\n`)
